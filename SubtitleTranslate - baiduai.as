...
*/
string GetTitle(){
    return "{$CP950=百度Ai机器翻译$}{$CP0=Bai Du Ai translate$}";
}
...
*/
string GetDesc(){
    return "https://cloud.baidu.com/doc/MT/s/4kqryjku9";
}
...
string JsonParse(string json){
    ...
    if (reader.parse(json, root)){//如果成功解析了json内容
        if(root.isObject()){//要求是对象模式
            ...
            //查找是否存在错误
            if(hasErrorInResult(keys,"1")){//如果发生了错误
                JsonValue errorCode = root["error"];//错误编号   "error_description" == keys[i] or "error" == keys[i]
                JsonValue errorMsg = root["error_description"];//错误信息描述
                ret = "error: " + errorCode.asString() + ", error_msg=" + errorMsg.asString();

            }else{//如果没发生错误
                JsonValue Result = root["access_token"];//取得翻译结果
                accessToKen = Result.asString();
                ret = accessToKen;
...
string JsonParse2(string json){
    ...
    if (reader.parse(json, root)){//如果成功解析了json内容
        if(root.isObject()){//要求是对象模式
            ...
            //查找是否存在错误
            //ret += "22222";
            if(hasErrorInResult(keys,"2")){//如果发生了错误
                JsonValue errorCode = root["error_code"];//错误编号   "error_description" == keys[i] or "error" == keys[i]
                JsonValue errorMsg = root["error_msg"];//错误信息描述
                ret = "error: " + errorCode.asString() + ", error_msg=" + errorMsg.asString();

            }else{//如果没发生错误
                JsonValue transResult = root["result"]["trans_result"];//取得翻译结果
                //ret +="33333";
                if(transResult.isArray()){//如果有翻译结果-必须是数组形式
                    //ret += "44444";
                for(int i = 0; i < transResult.size(); i++){
...
*/
bool hasErrorInResult(array<string> keys,string flag){
    bool result = false;
    if("1" == flag){
